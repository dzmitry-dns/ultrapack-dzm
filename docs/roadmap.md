...
- [x] **Project verify recipe:** `uverify`/`/up:make` read a per-project smoke recipe (cccc: `bun type-check`, `bun test:unit`, `bun run build`, dev smoke; integration gated on test DB; UI via Playwright/chrome MCP click-through) instead of guessing. *(merged v0.3.30, `docs/tasks/verify-recipe.md`)*
