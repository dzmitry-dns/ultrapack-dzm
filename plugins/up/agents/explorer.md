...
- Report incidental smells you pass on the traced path in `## Code smells` — file:line + one line. An observation with a pointer, not a proposed fix; don't go off-path hunting. You're read-only; the dispatcher fixes or records.
